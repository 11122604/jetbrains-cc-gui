...
export interface MessageListRevealHandle {
  ...
  /**
   * Ask for the next older page of the transcript, for callers that need a message
   * which is not loaded yet (history arrives one page of turns at a time).
   * Returns whether a request was actually sent.
   */
  loadEarlierPage: () => boolean;
  /** Whether older pages exist for the current session (i.e. paging can help). */
  canLoadEarlierPage: () => boolean;
}
