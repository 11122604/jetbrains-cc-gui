...
public class FindAiHistoryAction extends AnAction implements DumbAware {
    ...
    private void openHistoryInNewTab(Project project, JsonObject hit, String basePath) {
        ...
        String hitFilePath = hit.has("filePath") ? hit.get("filePath").getAsString() : "";
        ...
        ApplicationManager.getApplication().invokeLater(() -> {
            ...
            AppExecutorUtil.getAppExecutorService().execute(() -> {
                ...
                if (!hitFilePath.isEmpty()) {
                    // Fallback locator: a large tool parameter is rendered truncated, so
                    // the matched line may not exist in the DOM and the block holding
                    // that file is the most precise target left.
                    payload.addProperty("filePath", hitFilePath);
                }
